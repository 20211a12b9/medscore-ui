...
export const SendNotice = () => {
  ...
  const fetchInvoiceHistory = async () => {
    try {
      ...
      const response = await fetch(
        `${config.API_HOST}/api/user/getinvoicesbydistId/${distId}?page=${currentPage}&limit=${limit}`,
        {
          method: 'GET',
          // headers: {
  ...
  const handleUpdateRD = async (invoice) => {
    try {
      ...
      setLoading(true);
      const response = await fetch(`${config.API_HOST}/api/user/updateNotice`, {
        method: 'PUT',
        headers: {
          // 'Authorization':`Bearer ${localStorage.getItem('jwttoken')}`,
          'Content-Type': 'application/json',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-8">
            ...
            Search and Send Payment Reminder
            </h1>
          </div>
        ...
        <div>
          ...
          <button 
          onClick={() => navigate('/DetailedNoticesHistory')}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  Detailed Remainder History
                </h3>
                <p className="text-sm text-gray-600">
                  Click here to get Detailed Remainder History
                </p>
              </div>
            </div>
            <ChevronRight 
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-purple-50/50">
                    {["S.No", "Reminder Date", "Pharmacy Name", "Invoice No.", "Amount", "Invoice Date", "Due Date", "Delay", "Actions"].map((header) => (
