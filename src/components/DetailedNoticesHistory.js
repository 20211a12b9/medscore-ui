...
const DetailedNoticesHistory = () => {
    ...
    const fetchInvoices = async () => {
      try {
        ...
        const response = await fetch(
          `${config.API_HOST}/api/user/getDetailedinvoicesbydistId/${distId}?page=${currentPage}&limit=${limit}`,
          {
            method:'GET',
            // headers:{
 ...
 <div className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-sm shadow-lg">
  ...
  <div className="flex items-center gap-3 mb-6">
    ...
    <h2 className="text-2xl font-bold text-purple-900">Total Remainder History</h2>
  </div>
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-purple-50/50">
            {["S.No", "Reminder Date", "Pharmacy Name", "Invoice No.", "Amount", "Invoice Date", "Due Date", "Delay"].map((header) => (
...
export default DetailedNoticesHistory;
